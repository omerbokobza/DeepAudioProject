...
class Diffusion(L.LightningModule):
  ...
  def q_xt(
      self, x, p, block_size=None, sampling_eps_min=None, sampling_eps_max=None, preserve_prefix_guidance_tokens=True):
    ...
    if preserve_prefix_guidance_tokens:
        move_indices[:, :8] = False
    # Shuffle tokens in groups while preserving guidance tokens
    batch_size = x.shape[0]
    
    # # For each sample in the batch
    # for b in range(batch_size):
    #     # Shuffle first pair (tokens 0-1)
    #     if torch.rand(1) > 0.5:
    #         x[b, 0], x[b, 1] = x[b, 1].clone(), x[b, 0].clone()
    #     # Mask first 2 tokens with 5% probability
    #     if torch.rand(1) < 0.05:
    #         x[b, 0] = self.mask_index
    #         x[b, 1] = self.mask_index
            
    #     # Shuffle second pair (tokens 2-3) 
    #     if torch.rand(1) > 0.5:
    #         x[b, 2], x[b, 3] = x[b, 3].clone(), x[b, 2].clone()
    #     if torch.rand(1) < 0.05:
    #         x[b, 2] = self.mask_index
    #         x[b, 3] = self.mask_index
            
    #     # Shuffle last group (tokens 4-7)
    #     perm = torch.randperm(4)
    #     x[b, 4:8] = x[b, 4:8][perm]
    #     if torch.rand(1) < 0.05:
    #         x[b, 4] = self.mask_index
    #         x[b, 5] = self.mask_index
    #         x[b, 6] = self.mask_index
    #         x[b, 7] = self.mask_index
    ...
    if self.config.training.resample and \
      ...
      xt = self._resample_q_xt(x,
                               ...
                               sampling_eps_min,
                               sampling_eps_max,
                               preserve_prefix_guidance_tokens=preserve_prefix_guidance_tokens)
  ...
  def _semi_ar_sampler(
    ...
    GenFrom=True, NumTokensFrom=4, SpecificPt=None,Guidance_Dict={"genre": ["deephouse"]}):
   #GenFrom=True, NumTokensFrom=4, SpecificPt=None,Guidance_Dict={"genre": ["rock"], "mood": ["dark"], "instruments": ["electricguitar", "bass", "drums"]}):
