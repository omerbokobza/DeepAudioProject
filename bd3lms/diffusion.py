...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    if self.config.music:
      self.tokenizer = utils.EmptyTokenizer()
    else:
      self.tokenizer = tokenizer
    cfg_vocab = getattr(self.config.model, "vocab_size", None)
    if self.config.music:
      self.vocab_size = 2052 #2050
    else:
      self.vocab_size = cfg_vocab if cfg_vocab is not None else self.tokenizer.vocab_size
    ...
    if self.config.music:
        self.vocab_size = 2053 #2050
        # self.tokenizer.bos_token_id = 2052
        # self.tokenizer.eos_token_id = 2051
        # self.tokenizer.mask_token_id = 2049
        # self.tokenizer.pad_token_id = 2050
    ...
    if 'wandb' in config and config.wandb is not None:
      # We will load EnCodec model on-the-fly for sampling
      self.encodec_model = None
    #Load the Guidance Tokenizer
    self.guidance_tokenizer = utils.TagsTokenizer()
  ...
  def to(self, *args, **kwargs):
    ...
    if hasattr(self, 'encodec_model') and self.encodec_model is not None:
      self.encodec_model = self.encodec_model.to(*args, **kwargs)
      print(f"ENCODEC MODEL TO {self.device}")
    return self
  ...
  def training_step(self, batch, batch_idx):
    del batch_idx
    # # Print first and last 20 tokens of each item in batch
    # input_ids = batch['input_ids']
    # print("First 20 tokens of each sequence:")
    # print(input_ids[:, :20])
    # print("\nLast 20 tokens of each sequence:") 
    # print(input_ids[:, -20:])
    # # Get min and max values from input_ids
    # min_val = input_ids.min().item()
    # max_val = input_ids.max().item()
    # print(f"\nMin value in batch: {min_val}")
    # print(f"Max value in batch: {max_val}")
    # Check if 2050 appears in input_ids
    # Check if 2051 appears in input_ids
    # if 2050 in input_ids:
    #     print(f"Found 2050 in input_ids")
    #     # Get last 200 tokens for each sequence containing 2051
    #     for seq in input_ids:
    #         print(seq)
    #         if 2051 in seq:
    #             # Find first occurrence of 2051
    #             start_idx = (seq == 2051).nonzero()[0].item()
    #             # Print from first 2051 to end
    #             if start_idx > 200: # if the 2051 is not the first token
    #                 print(seq[start_idx-200:start_idx+200])
    #             else:
    #                 print(seq[0:start_idx+200])
    #             exit(1)
    # # Check if 2051 appears in input_ids
    # if 2051 in input_ids:
    #     print(f"Found 2051 in input_ids")
    #     # Get last 200 tokens for each sequence containing 2051
    #     for seq in input_ids:
    #         print(seq[-400:-1])
    # # Check if 2052 appears in input_ids 
    # if 2052 in input_ids:
    #     # Get last 200 tokens for each sequence containing 2052
    #     for seq in input_ids:
    #         if 2052 in seq:
    #             print(seq[0:400])
  ...
  def configure_optimizers(self):
    ...
    #  See: https://github.com/Lightning-AI/pytorch-lightning/issues/5558s
  ...
  def _resample_q_xt(
      self, x, xt, move_indices, p, block_size, sampling_eps_min, sampling_eps_max, preserve_prefix_guidance_tokens=True):
    ...
    while (perc_masked < sampling_eps_min).any() or \
      ...
      if preserve_prefix_guidance_tokens:
          move_indices[:, :8] = False
  ...
  def q_xt(
    ...
    Args:
      ...
      p: float torch.Tensor with shape (batch_size, 1) or (batch_size, seq_len).
    ...
    if self.config.music:
      # --- New logic for chunk-based masking when in music mode ---
      batch_size, seq_len = x.shape
      
      # Assert divisibility by 4 for chunk-based masking
      assert seq_len % 4 == 0, "Sequence length must be divisible by 4 for chunk-based masking."
      
      # Determine number of chunks (each chunk has 4 tokens)
      num_chunks = seq_len // 4
      
      # Make one decision per chunk
      chunk_rand_vals = torch.rand((batch_size, num_chunks), device=x.device)
      
      # p has shape (batch_size, seq_len). We need one probability per chunk.
      # We'll take the probability of the first token in each chunk.
      p_chunks = p[:, ::4] # Shape: (batch_size, num_chunks)
      
      # Compare with broadcasted p
      chunk_move_mask = chunk_rand_vals < p_chunks
      
      # Repeat each chunk decision 4 times to cover all tokens in the chunk
      move_indices = chunk_move_mask.repeat_interleave(4, dim=1)  # Shape: (batch_size, seq_len)
      
      # Ensure the first 8 tokens (2 chunks) will not be masked
      move_indices[:, :8] = False
      # Apply the mask
      xt = torch.where(move_indices, self.mask_index, x)
      ...
      # Original logic for non-music mode
  ...
  def _semi_ar_sampler(
    ...
    for stride_num in tqdm(range(num_strides)):
      ...
      if stride_num == 0:
        ...
        x_accum[:, 0:4] = self.tokenizer.bos_token_id * torch.ones(4, device=x_accum.device, dtype=x_accum.dtype)
      else:
        if mdlm_semi_ar:
      ...
      if x_accum.shape[1] > 256:
        ...
        if (stop and not self.config.sampling.var_length) \
          ...
          break
    print(f"X ACCUM END: {x_accum[:, -256:]}")
    print(f"X ACCUM START : {x_accum[:, 0:320]}")
    print(f"X Accum after 240: {x_accum[:, 240:560]}")
  ...
  def _check_stop_conds(self, x):
    ...
    if not isinstance(self.tokenizer, utils.EmptyTokenizer):
    ...
    if self.config.sampling.var_length:
      ...
      if not isinstance(self.tokenizer, utils.EmptyTokenizer) and len(torch.where(x == self.tokenizer.eos_token_id)[0]) > 1:
        stop = True
  ...
  @torch.no_grad()
  def _semi_ar_sampler(
    self, n_samples, num_steps, num_strides, seqlen, context_size=4096,
    #GenFrom=True, NumTokensFrom=0, SpecificPt=None,Guidance_Dict={"genre": ["reggae"]}):
    GenFrom=False, NumTokensFrom=0, SpecificPt=None,Guidance_Dict={"genre": ["rock"], "mood": ["dark"], "instruments": ["electricguitar", "bass", "drums"]}):
    print(f"- DEBUG - context size: {context_size}")
    ...
    # The fix is to ensure x_accum is ALWAYS created with dtype=torch.long
    
    if GenFrom:
        # === A. Create x_accum from pre-tokens ===
        print("✅ Bypass enabled: Generating from pre-tokens.")
        
        # 1. Determine the source .pt file path
        if SpecificPt:
            pt_file_path = SpecificPt
            if not os.path.exists(pt_file_path):
                raise FileNotFoundError(f"The specified file does not exist: {SpecificPt}")
        else:
            musicnet_dir = '/home/tzlillev/LLadaSMDM/MusicNet_32khz_Tokens' 
            if not os.path.isdir(musicnet_dir):
                raise FileNotFoundError(f"The directory '{musicnet_dir}' does not exist.")
            pt_files = [f for f in os.listdir(musicnet_dir) if f.endswith('.pt')]
            if not pt_files:
                raise FileNotFoundError(f"No .pt files found in '{musicnet_dir}'.")
            pt_file_path = os.path.join(musicnet_dir, random.choice(pt_files))
        
        print(f"Loading tokens from: {pt_file_path}")
        
        # 2. Load tokens and ensure they are torch.long
        prompt_tokens_full = torch.load(pt_file_path, map_location=self.device)
        if prompt_tokens_full.ndim > 1:
            prompt_tokens_full = prompt_tokens_full[0]
        
        # FIX #1: Ensure loaded tokens and BOS tokens are long integers
        prompt_tokens = prompt_tokens_full[:NumTokensFrom].long() 
        bos_tokens = torch.full((4,), self.tokenizer.bos_token_id, device=self.device, dtype=torch.long)
        if Guidance_Dict is not None:
          guidance_tokens = self.guidance_tokenizer.tokenize(Guidance_Dict)
          print(f"Guidance Tokens: {guidance_tokens}")
          guidance_tensor = torch.tensor(guidance_tokens["tokens"], dtype=torch.long, device=self.device)
          initial_sequence = torch.cat((guidance_tensor, bos_tokens, prompt_tokens))
        else:
          initial_sequence = torch.cat((bos_tokens, prompt_tokens))
        print(f"first 40 tokens of initial sequence: {initial_sequence[0:40]}")
      #  print(f"Initial Sequence: {initial_sequence}")
        # 3. Create the full tensor and place the initial sequence at the start
        initial_len = initial_sequence.shape[0]
        if initial_len > seqlen:
            print(f"⚠️ Warning: Prompt length ({initial_len}) is greater than target sequence length ({seqlen}). Truncating.")
            initial_len = seqlen
            initial_sequence = initial_sequence[:initial_len]

        # FIX #2: Create x_accum with dtype=torch.long
        x_accum = torch.full((n_samples, seqlen), self.mask_index, dtype=torch.long, device=self.device)
        x_accum[:, :initial_len] = initial_sequence

    else:
        # === B. Create a standard masked x_accum for generation from scratch ===
        print("ℹ️ Standard generation: starting from scratch.")
        # FIX #3: Ensure x_accum is created with dtype=torch.long for the standard path too
        x_accum = torch.full((n_samples, seqlen), self.mask_index, dtype=torch.long, device=self.device)
        x_accum[:, 0:4] = self.tokenizer.bos_token_id
    ...
    for stride_num in tqdm(range(num_strides)):
        ...
        # If guidance is present, ensure first 8 tokens are always in context
        if Guidance_Dict is not None:
            start_idx = 0 if start_idx < 8 else start_idx
        
        end_idx = min(end_idx, seqlen)
        ...
        if mdlm_semi_ar and stride_num > 0:
            ...
            # Keep first 8 tokens in context if guidance present
            if Guidance_Dict is not None:
                fwd_idx = torch.cat([torch.arange(0, 8, device=self.device), fwd_idx])
                print(f"\nStride {stride_num} - First 8 tokens in context: {x_accum[0, :8]}")

        #print(f"\nStride {stride_num} - Window indices: {fwd_idx[0].item()} to {fwd_idx[-1].item()}")
        #print(f"Current window content: {x_accum[0, fwd_idx]}")
        ...
        for i in range(num_steps):
            if self.mask_index not in x_accum[:, fwd_idx]:
                break

            t = timesteps[i] if not self.config.sampling.first_hitting else 1 * (np.random.rand() ** (1 / (x_accum[:, fwd_idx] == self.mask_index).sum(-1).item()))
            ...
            x_accum[:, fwd_idx] = x_next.long() # Ensure the updated part remains long

    print(f"X ACCUM END: {x_accum[:, -400:]}")
   # print(f"X ACCUM START : {x_accum[:, 0:320]}")
    return x_accum, sampling_steps